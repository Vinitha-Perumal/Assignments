...
void TestCases () {
   ...
   for (int i = 0; i < numOfTestCases; i++) {
      ...
      printf ("Addition: %s\n", IsEqual (Op_Add (num1[i], num2[i]), expectedAddOut) ? "Pass" : "Fail");

      // Subtraction TestCase
      ComplexNumber expectedSubOut = { num1[i].Real - num2[i].Real, num1[i].Img - num2[i].Img };
      printf ("Subtraction: %s\n", IsEqual (Op_Sub (num1[i], num2[i]), expectedSubOut) ? "Pass" : "Fail");
      ...
      printf ("Multiplication: %s\n", IsEqual (Op_Mul (num1[i], num2[i]), expectedMulOut) ? "Pass" : "Fail");

      // Conjugate TestCase
      ComplexNumber expectedConjOut = { num1[i].Real, -num1[i].Img };
      printf ("Conjugate: %s\n", IsEqual (Op_Conj (num1[i]), expectedConjOut) ? "Pass" : "Fail");

      // Modulus TestCase
      double expectedModOut = sqrt (num1[i].Real * num1[i].Real + num1[i].Img * num1[i].Img);
      printf ("Modulus: %s\n", Op_Mod (num1[i]) == expectedModOut ? "Pass" : "Fail");
   }
}
